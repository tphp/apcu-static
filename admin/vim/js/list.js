var set_html_value, set_html_affer, set_square, resize_init, tree_value_alls = {};
$(function () {
    var body = $('body');
            ...
            $(g_tr + "[pkmd5='" + md5 + "'] td").each(function () {
                ...
                if(objv[key] !== undefined) {
                    ...
                    if(type === undefined || type === '') {
                        ...
                        //     }
                        // }
                        _this.find(".layui-table-cell").attr("value", src[md5][key]).attr("title", title).find(".value").html(objv[key]);
                    }else if(type === 'status'){
                        var name = key + '[' + md5 + ']';
                        var data = {};
    ...
    function add_table_tr(tr_md5s) {
        ...
        td_first.parent().each(function () {
            $(this).attr("data-posturl", base_url + ".edit?pk=" + encodeURI(JSON.stringify([$(this).attr("pk")])));
        });
                                    ...
                                    if (ltc.attr("value") === _tv) {
                                        ...
                                        $.post(_posturl, data, function (result) {
                                            ...
                                            if (result.code === 1) {
                                                ...
                                                if(retval !== _tv){
                                                    alert_error("格式有误，数据已调整！", _tt);
                                                }
                                            } else {
                                                alert_error(result.msg, _tt);
                                            }
                                            _t.attr("data-flag", "false");
                                            return result;
                                        });
                ...
                td_first.each(function () {
                    ...
                    if(__tp.find(">td[data-type='" + type + "']").length <= 0) {
                        ...
                        for(var i in djson){
                            ...
                            if(iobj instanceof Object){
                                iname = iobj.name;
                                strhtml += 'data-url="' + iobj.url + '" data-click="true" ';
                                objkey = iobj.key;
                        ...
                        if (result.code === 1) {
                            ...
                            if(is_tree === 'true'){
                                ...
                                for(var i in src) {
                                    ...
                                    if(src[i][parent] === parentid && pkobjsize > 0){
                                        dt_edit['src'][i] = src[i];
    ...
    function postCheckList(type, obj){
        ...
        if(type === 'deletes'){
            layui_delete(base_url + "." + type, '确定删除所选？', checklist, [], md5list);
        }else if(type === 'clear'){
            layui_clear(base_url + ".clear");
        }else if(type === 'edits'){
            layui_edit(base_url + ".edit?handle_id=" + $(obj).attr("data-hdkey") + "&pks=" + encodeURI(checkjsonstr), $(obj).html());
        }else if(type === 'add'){
            layui_edit(base_url + ".add", "新增", type);
        }
        return checklist;
    }
    ...
    function postCheckOper(obj, type){
        ...
        o.parent().parent().parent().each(function () {
            ...
            tit = $(this).find("td[key='" + oper_title + "'] .layui-table-cell").attr("value");
            if(tit !== undefined){
        ...
        if(type === 'delete'){
            layui_delete(base_url + "." + type, '确定删除（' + tit + '）？', checklist, config, md5list);
        }else if(type === 'add') {
            var titin = '新增';
            if(tit !== ''){
                titin += g_next_name_flag + tit;
            }
            layui_edit(base_url + ".add?pk=" + encodeURI(checkjsonstr), titin, 'add', config);
        }else if(type === 'edit'){
            var titin = '编辑';
            if(tit !== ''){
                titin += g_next_name_flag + tit;
            }
            layui_edit(base_url + ".edit?pk=" + encodeURI(checkjsonstr), titin, 'edit', config, md5list);
        }else if(type === 'view'){
            var titin = '查看';
            if(tit !== ''){
                titin += g_next_name_flag + tit;
            }
            layui_edit(base_url + ".view?pk=" + encodeURI(checkjsonstr), titin, type, config);
        }else if(type === 'url'){
            if(checklist.length > 0) {
                try {
    ...
    function postCheckClick(obj){
        ...
        if(th_click_url !== undefined && th_click_url !== "") {
            ...
            var clickurl = th_click_url + cflag + "key=" + encodeURI(th_click_key) + "&value=" + encodeURI(th_click_value);
            ...
            var checkjsonstr = JSON.stringify(checklist);
            var posturl = base_url + ".edit?pk=" + encodeURI(checkjsonstr);
            var layerconfig = {
                type: 2
                , title: title
                , area: [h_w + 'px', h_h + 'px']
